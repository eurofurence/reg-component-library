...
const spinnerColor = (variant: Variant) => {
	switch (variant) {
		...
		case 'button': return 'var(--color-grays-000)'
		case 'inverted-card-button': return 'var(--color-brand-2-500)'
	}
}

const Spinner = ({ variant = 'default' }: SpinnerProps) => <svg
...
>
	<mask id="spinner">
		<rect fill="rgba(255,255,255,0.1)" x={-SPINNER_RADIUS} y={-SPINNER_RADIUS} width={2 * SPINNER_RADIUS} height={2 * SPINNER_RADIUS}/>
		<path fill="white" d={`
			M 0,0
			V ${-SPINNER_RADIUS}
